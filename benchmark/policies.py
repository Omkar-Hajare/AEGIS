...
class AdaptivePolicyAdapter(BenchmarkPolicy):
    ...
    def select_evictions(
        ...
        miss_rate = (self._misses / self._requests) if self._requests > 0 else 1.0
        backend_lat = (
            (self._backend_latency_sum / self._misses)
            if self._misses > 0
            else (event.backend_latency_ms if event else 5.0)
        )
