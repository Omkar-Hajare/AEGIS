...
def render_request_simulator_view():
    ...
    guide_html = (
        ...
        f'<div style="font-size: 12.5px; color: {c["text"]}; margin-top: 3px;">'
    ...
    with col_result:
        ...
        if last_res:
            ...
            res_card_html = (
                ...
                f'<div style="font-size: 15px; font-weight: 800; color: {c["emerald"]};">{last_res.get("status_code", 200)} OK</div>'
                f'</div>'
                f'<div style="background: {c["card_bg_elevated"]}; padding: 8px; border-radius: 6px; border: 1px solid {c["card_border"]};">'
                f'<span class="muted" style="font-size: 10px; font-weight: 700; text-transform: uppercase; display: block;">Target Key</span>'
                f'<div style="font-size: 12px; font-weight: 700; color: {c["text"]}; word-break: break-all; margin-top: 3px;">{last_res.get("target_key")}</div>'
                f'</div>'
                f'</div>'
                f'<p style="font-size: 12px; color: {c["text_muted"]}; margin: 0 0 10px 0;">{verdict_explanation}</p>'
