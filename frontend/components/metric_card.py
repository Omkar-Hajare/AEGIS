...
def render_metric_card(
    ...
    # Ensure footer slot stability
    delta_html = (
        f'<div style="font-size:11.5px;font-weight:600;color:{actual_delta_color};display:flex;align-items:center;gap:4px;">'
        f'{delta}</div>'
        if delta
        else ""
    )
    ...
    tag_html = (
        f'<span style="font-size:9.5px;font-weight:700;background:{"rgba(255,255,255,0.06)" if c["is_dark"] else "rgba(0,0,0,0.04)"};color:{c["text_muted"]};border:1px solid {c["card_border"]};border-radius:4px;padding:2px 6px;letter-spacing:0.5px;text-transform:uppercase;white-space:nowrap;">{tag}</span>'
        if tag
        else ""
    )
    ...
    if effective_progress is not None:
        ...
        bar_html = (
            f'<div style="background:{bar_bg};height:4px;border-radius:2px;overflow:hidden;margin-top:6px;margin-bottom:2px;">'
            f'<div style="background:{p_col};width:{p_pct}%;height:100%;border-radius:2px;"></div>'
            f'</div>'
        )

    subtitle_html = (
        f'<div style="color:{c["text_muted"]};font-size:11px;font-weight:500;line-height:1.3;">{subtitle}</div>'
        if subtitle
        else '<div style="height:14px;"></div>'
    )

    html = (
        f'<div class="hero-card metric-card" style="padding:20px 22px;min-height:148px;height:100%;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;">'
        f'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:8px;">'
        f'<div style="display:flex;align-items:center;">'
        f'<span style="color:{c["text_muted"]};font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;line-height:1.2;">{title}</span>'
        f'{tooltip_html}'
        f'</div>'
        f'{tag_html}'
        f'</div>'
        f'<div style="margin-bottom:auto;">'
        f'<div style="font-size:26px;font-weight:800;color:{c["text"]};letter-spacing:-0.6px;line-height:1.15;margin-top:2px;">{value}</div>'
        f'</div>'
        f'<div style="margin-top:8px;display:flex;flex-direction:column;gap:3px;">'
        f'{delta_html}'
        f'{bar_html}'
        f'{subtitle_html}'
        f'</div>'
